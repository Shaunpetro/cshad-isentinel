...
/**
 * Fetch verified journalists from Supabase profiles table
 */
async function fetchVerifiedJournalists(): Promise<Journalist[]> {
  console.log(TAG, 'Fetching verified journalists');

  const { data, error } = await supabase
    .from('profiles')
    .select('id, username, email, verified_journalist')
    .eq('verified_journalist', true)
    .eq('status', 'active');

  if (error) {
    console.error(TAG, 'Error fetching journalists:', error);
    return [];
  }

  return (data || []).map((profile: any) => ({
    id: profile.id,
    name: profile.username || profile.email?.split('@')[0] || 'Journalist',
    handle: profile.username ? `@${profile.username}` : undefined,
    avatarUrl: undefined,
    isVerified: true,
    verificationType: 'journalist' as const,
    outlet: undefined,
    specialty: undefined,
    recentReports: 0,
  }));
