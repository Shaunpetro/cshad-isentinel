    ...
    if (includeTips) {
      ...
      if (tipsError) {
        ...
        for (const tip of tipsData) {
          // Try to get location from coordinates or extract from description
          const location = geocodeFromMultipleSources(
            null, // Tips don't have location_name column
            null,
            tip.description,
            tip.latitude,
            tip.longitude
  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        // Tips don't have location_name column - extract from description or use coords
        const location = geocodeFromMultipleSources(
          null,
          null,
