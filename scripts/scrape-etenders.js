...
function parseDate(dateStr) {
  ...
  if (timeMatch) {
    ...
    clean = clean.substring(0, timeMatch.index).replace(/[-–]\s*$/, '').trim();
  }

  // Numeric dd/mm/yyyy, dd-mm-yyyy, or dd.mm.yyyy — checked BEFORE the generic
  // Date constructor below, because JS's Date parser is locale-dependent for
  // ambiguous numeric dates and can silently read dd-mm-yyyy as mm-dd-yyyy.
  // This also fixes hyphen-separated dates, which the old split-only-on-
  // whitespace-or-slash fallback could never match (it always fell through to
  // "today" for any dd-mm-yyyy formatted date).
  ...
  // Month-name formats (e.g. "16 June 2026") are unambiguous, safe to hand to
  // the built-in Date constructor.
  ...
  // dd MM yyyy fallback (space-or-slash separated, no recognizable month name)
